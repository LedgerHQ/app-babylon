...
bool copy_transaction_parameters(
    ...
    memset(destination_address_extra_data, 0,
    ...
    // aligned to right input {0xEE, 0x00, 0xFF} should be stored like {0x00,
    ...
    memcpy(G_swap_state.destination_address, destination_address,
           sizeof(G_swap_state.destination_address));

    // if destination_address_extra_id is given, we use the first byte to
    // determine if we use the normal swap protocol, or the one for cross-chain
    // swaps
    if (destination_address_extra_data[0] == 0) {
        ...
        explicit_bzero(G_swap_state.payin_extra_id,
                       sizeof(G_swap_state.payin_extra_id));
    } else if (destination_address_extra_data[0] == 2) {
        G_swap_state.mode = SWAP_MODE_CROSSCHAIN;

        // we expect exactly 33 bytes. Guard against future protocol changes, as
        // the following code might need to be revised in that case
        LEDGER_ASSERT(sizeof(G_swap_state.payin_extra_id) == 33,
                      "Unexpected payin_extra_id size");

        memcpy(G_swap_state.payin_extra_id, destination_address_extra_data,
               sizeof(G_swap_state.payin_extra_id));
    } else {
        // Since we cannot return an error status word here, we mark the swap
        // state as invalid and will return an error later, once an attempt is
        // made to sign.
