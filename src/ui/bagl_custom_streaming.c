...
const bagl_element_t* ux_layout_paging_prepro_common_streaming(
    const bagl_element_t* element, const char* title, const char* text) {
    ...
    switch (element->component.userid) {
        ...
        case 0x10:
            ...
            if (title) {
                ...
                SPRINTF(G_ux.string_buffer, "%d/%d",
        ...
        case 0x13: {
            ...
            if (lineidx < UX_LAYOUT_PAGING_LINE_COUNT &&
                G_ux.layout_paging.lengths[lineidx]) {
                SPRINTF(G_ux.string_buffer, "%.*s",
                        // avoid overflow
                        MIN(sizeof(G_ux.string_buffer) - 1,
                            G_ux.layout_paging.lengths[lineidx]),
...
static void ux_layout_paging_prev(ux_layout_paging_redisplay_t redisplay) {
    if (G_ux.layout_paging.current == 0) {
        ...
        // display previous page, count the number of char to fit in the
        // previous page
        G_ux.layout_paging.current--;
        redisplay(G_ux.stack_count - 1);
    }
...
STATIC_IF_NOT_INDEXED unsigned int
ux_layout_paging_button_callback_common_streaming(
...
static const bagl_element_t* ux_layout_paging_prepro_by_addr(
...
void ux_layout_paging_init_common_streaming(
    unsigned int stack_slot, const char* text,
    ux_layout_paging_redisplay_t redisplay) {
    bagl_font_id_e font_id;

    // At this very moment, we don't want to get rid of the format, but keep
    // the one which has just been set (in case of direction backward or
    // forward).
    ...
    switch (ux_flow_direction()) {
        case FLOW_DIRECTION_BACKWARD:
            ...
            // This step must be performed after the 'ux_layout_paging_reset'
            // call, thus we cannot mutualize the call with the one in the
            // 'forward' case.
    ...
    G_ux.layout_paging.count = ux_layout_paging_compute(
        text, -1UL, &G_ux.layout_paging, font_id);  // at least one page

    // perform displaying the last page as requested (-1UL in prevstep hook does
    // this)
    if (G_ux.layout_paging.count &&
        G_ux.layout_paging.current > G_ux.layout_paging.count - 1UL) {
        G_ux.layout_paging.current = G_ux.layout_paging.count - 1;
    }
...
ux_layout_paging_button_callback_by_addr_streaming(
    ...
    return ux_layout_paging_button_callback_common_streaming(
        button_mask, button_mask_counter,
...
#if defined(HAVE_INDEXED_STRINGS)
    text =
        get_ux_loc_string(((const ux_loc_layout_params_t*)params)->index + 1);
#else   // defined(HAVE_INDEXED_STRINGS)
    text = ((const ux_layout_paging_params_t*)params)->text;
#endif  // defined(HAVE_INDEXED_STRINGS)
    ux_layout_paging_redisplay_common(
        stack_slot, text, ux_layout_paging_button_callback_by_addr_streaming,
