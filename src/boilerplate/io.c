...
// The initial value does not matter, as only the difference between timeframes
// is used.
uint16_t G_ticks;
...
// set to true when the "Processing..." screen is shown, in order for the
// dispatcher to know if the UX is not in idle state at the end of a command
// handler.
...
#ifdef HAVE_BAGL
UX_STEP_NOCB(ux_processing_flow_1_step, pn,
             {&C_icon_processing, "Processing..."});
...
void io_clear_processing_timeout() { G_is_timeout_active.processing = false; }
    ...
    switch (G_io_seproxyhal_spi_buffer[0]) {
        ...
        case SEPROXYHAL_TAG_TICKER_EVENT:
            ...
            if (G_is_timeout_active.processing &&
                G_ticks - G_processing_timeout_start_tick >=
                    PROCESSING_TIMEOUT_TICKS) {
                io_clear_processing_timeout();
            ...
            if (G_is_timeout_active.interruption &&
                G_ticks - G_interruption_timeout_start_tick >=
                    INTERRUPTION_TIMEOUT_TICKS) {
                io_clear_interruption_timeout();

                // TODO: It would be better to have the dispatcher be notified
                // somehow.
                //       This would require some tampering with the io_exchange
                //       in process_interruption.
                THROW(EXCEPTION_IO_RESET);
            }
...
void io_add_to_response(const void* rdata, size_t rdata_len) {
    if (G_output_len >= IO_APDU_BUFFER_SIZE - 2) {
        ...
        write_u16_be(G_io_apdu_buffer, IO_APDU_BUFFER_SIZE - 2,
...
void io_reset_response() { G_output_len = 0; }
...
int io_send_sw(uint16_t sw) { return io_send_response(NULL, 0, sw); }
