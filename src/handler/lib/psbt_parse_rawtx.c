...
typedef struct {
    struct parse_rawtx_state_s*
        parent_state;             // subparsers can access parent's state
    unsigned int scriptsig_size;  // max 10_000 bytes
    unsigned int
        scriptsig_counter;  // counter of scriptsig bytes already received
...
typedef struct {
    ...
    unsigned int
        scriptpubkey_counter;  // counter of scriptpubkey bytes already received
...
typedef struct parse_rawtx_state_s {
    ...
    union {
        // since the parsing stages of inputs, outputs and witnesses are
        // disjoint, we reuse the same space in memory
        struct {
        ...
        struct {
            ...
            unsigned int wit_stack_el_counter;  // index of the stack element in
                                                // the witness field
            unsigned int cur_wit_stack_elements;  // number of stack elements
            unsigned int
                cur_wit_elem_len;  // size of the current stack elements
            unsigned int cur_wit_el_bytes_read;  // number of bytes read of the
                                                 // current element
...
static int parse_rawtxinput_txid(parse_rawtxinput_state_t* state,
                                 buffer_t* buffers[2]) {
    uint8_t txid[32];
    bool result = dbuffer_read_bytes(buffers, txid, 32);
    if (result) {
        crypto_hash_update(&state->parent_state->hash_context->header, txid,
                           32);
    }
    return result;
}
...
// TODO: shares logic with the previous method; try to factor out the shared
// code
static int parse_rawtxinput_vout(parse_rawtxinput_state_t* state,
                                 buffer_t* buffers[2]) {
    uint8_t vout_bytes[4];
    bool result = dbuffer_read_bytes(buffers, vout_bytes, 4);
    if (result) {
        crypto_hash_update(&state->parent_state->hash_context->header,
                           vout_bytes, 4);
    }
    return result;
}

static int parse_rawtxinput_scriptsig_size(parse_rawtxinput_state_t* state,
                                           buffer_t* buffers[2]) {
...
static int parse_rawtxinput_scriptsig_init(parse_rawtxinput_state_t* state,
                                           buffer_t* buffers[2]) {
    (void)buffers;
...
static int parse_rawtxinput_scriptsig(parse_rawtxinput_state_t* state,
                                      buffer_t* buffers[2]) {
    uint8_t data[32];
    ...
    while (true) {
        ...
        // We read in chunks of at most 32 bytes, so that we can always
        // interrupt with less than 32 unparsed bytes
        ...
        crypto_hash_update(&state->parent_state->hash_context->header, data,
                           data_len);
...
static int parse_rawtxinput_sequence(parse_rawtxinput_state_t* state,
                                     buffer_t* buffers[2]) {
...
static int parse_rawtxoutput_value(parse_rawtxoutput_state_t* state,
                                   buffer_t* buffers[2]) {
    ...
    if (result) {
        ...
        if (state->parent_state->output_index != -1) {
            unsigned int relevant_output_index =
                (unsigned int)state->parent_state->output_index;
...
static int parse_rawtxoutput_scriptpubkey(parse_rawtxoutput_state_t* state,
                                          buffer_t* buffers[2]) {
    uint8_t data[32];

    while (true) {
        int remaining_len =
            state->scriptpubkey_size - state->scriptpubkey_counter;
...
static int parse_rawtx_version(parse_rawtx_state_t* state,
                               buffer_t* buffers[2]) {
...
// Checks if this transaction is serialized according to bip144 (segwit), that
// is, it has a 0x00 marker followed by a 0x01 flag where the input count would
// be expected in the legacy serialization. The marker and flag are read from
// the buffers. Does not read any bytes from the buffers if the transaction is
// in the legacy serialization format. The marker and flag (if present) are not
// added to the hash computation.
static int parse_rawtx_check_segwit(parse_rawtx_state_t* state,
                                    buffer_t* buffers[2]) {
    ...
    if (first_byte != 0) {
        ...
        // Segwit format, the first byte is 0x00 and the next should be the 0x01
        // flag.
        ...
        if (flag != 0x01) {
            PRINTF(
                "Unexpected flag while parsing a segwit transaction: %02x.\n",
                flag);
            return -1;
        }
...
static int parse_rawtx_input_count(parse_rawtx_state_t* state,
                                   buffer_t* buffers[2]) {
    uint64_t n_inputs;
...
static int parse_rawtx_inputs_init(parse_rawtx_state_t* state,
                                   buffer_t* buffers[2]) {
    (void)buffers;

    state->in_counter = 0;

    parser_init_context(&state->input_parser_context,
                        &state->input_parser_state);

    state->input_parser_state.parent_state = state;
    return 1;
}

static int parse_rawtx_inputs(parse_rawtx_state_t* state,
                              buffer_t* buffers[2]) {
    while (state->in_counter < state->n_inputs) {
        while (true) {
            bool result =
                parser_run(parse_rawtxinput_steps, n_parse_rawtxinput_steps,
                           &state->input_parser_context, buffers, pic);
            if (result != 1) {
...
static int parse_rawtx_output_count(parse_rawtx_state_t* state,
                                    buffer_t* buffers[2]) {
    uint64_t n_outputs;
...
static int parse_rawtx_outputs_init(parse_rawtx_state_t* state,
                                    buffer_t* buffers[2]) {
    (void)buffers;

    state->out_counter = 0;
    parser_init_context(&state->output_parser_context,
                        &state->output_parser_state);

    state->output_parser_state.parent_state = state;
    return 1;
}

static int parse_rawtx_outputs(parse_rawtx_state_t* state,
                               buffer_t* buffers[2]) {
    while (state->out_counter < state->n_outputs) {
        while (true) {
            bool result =
                parser_run(parse_rawtxoutput_steps, n_parse_rawtxoutput_steps,
                           &state->output_parser_context, buffers, pic);
            if (result != 1) {
                return result;  // stream exhausted, or error
            } else {
...
static int parse_rawtx_witnesses_init(parse_rawtx_state_t* state,
                                      buffer_t* buffers[2]) {
    (void)buffers;
...
static int parse_rawtx_witnesses(parse_rawtx_state_t* state,
                                 buffer_t* buffers[2]) {
    if (!state->is_segwit) {
    ...
    while (state->wit_counter < state->n_inputs) {
        ...
        if (!state->is_cur_wit_stack_elements_read) {
            ...
            state->cur_wit_stack_elements =
                (unsigned int)cur_wit_stack_elements;
        ...
        while (state->wit_stack_el_counter < state->cur_wit_stack_elements) {
            // read the length of the current stack element (if not already
            // read)
            ...
            while (state->cur_wit_el_bytes_read < state->cur_wit_elem_len) {
                uint8_t data[32];
                int remaining_len =
                    state->cur_wit_elem_len - state->cur_wit_el_bytes_read;
...
static int parse_rawtx_locktime(parse_rawtx_state_t* state,
                                buffer_t* buffers[2]) {
...
const int n_parse_rawtx_steps =
    sizeof(parse_rawtx_steps) / sizeof(parse_rawtx_steps[0]);
...
static void cb_process_data(buffer_t* data, void* cb_state) {
    ...
    if (state->parser_error) {
        // there was already a parsing error, ignore any additional data
        // received
        return;
    }
    ...
    int result = parser_run(parse_rawtx_steps, n_parse_rawtx_steps,
                            &state->parser_context, buffers, pic);
    if (result == 0) {
...
int call_psbt_parse_rawtx(dispatcher_context_t* dispatcher_context,
                          ...
                          const uint8_t* key, int key_len, int output_index,
    ...
    int res = call_get_merkleized_map_value_hash(dispatcher_context, map, key,
                                                 key_len, value_hash);
    if (res < 0) {
        return -1;
    }
    ...
    res = call_stream_preimage(dispatcher_context, value_hash, NULL,
                               cb_process_data, &flow_state);
