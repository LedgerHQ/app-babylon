...
// An example parser that reads an uint32_t, an array of 8 bytes, and an
// uint8_t.
typedef struct {
    uint32_t a;
    uint8_t b[8];
...
const parsing_step_t parse_ABC_steps[] = {
    (parsing_step_t)parse_A, (parsing_step_t)parse_B, (parsing_step_t)parse_C};
...
// A parser similar to parse_ABC, but always generates an error in the second
// parsing step
...
static void test_parser_oneshot_from_store(void** state) {
    ...
    int result = parser_run(parse_ABC_steps, n_ABC_STEPS, &parser_context,
                            buffers, NULL);

    assert_int_equal(result, 1);
    assert_int_equal(parser_context.cur_step,
                     n_ABC_STEPS);  // parsing completed
...
static void test_parser_oneshot_part_store_part_stream(void** state) {
    ...
    buffer_t store_buf =
        buffer_create(store, 5 + 4 + 2);  // size includes the initial zeros
...
static void test_parser_stream_ends(void** state) {
    ...
    assert_int_equal(parser_context.cur_step,
                     1);  // second parsing step was not completed
...
static void test_parser_continue_partial(void** state) {
    ...
    // this tests completes the parsing from the point it left in the previous
    // test
    uint8_t store[32] = {
        0xb0,
...
static void test_parser_error(void** state) {
    ...
    // in this test, the second parsing stap causes an error; that should be
    // returned
    // clang-format off
    uint8_t store[32] = {
        0xa0, 0xa1, 0xa2, 0xa3,
    ...
    int result = parser_run(parse_ABC_error_steps, n_ABC_STEPS, &parser_context,
                            buffers, NULL);
    ...
    // Check expected parsing results
    assert_int_equal(parser_state.a,
                     0xa0a1a2a3);  // a should have been parsed correctly
}
